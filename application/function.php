...
/**
 * @Author: 陈静
 * @Date: 2018/03/28 15:21:44
 * @Description: 正则匹配验证身份证
 * @param $card
 * @return bool
 */
function is_id_card($card)
{
    $chars = '/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|[xX])$/';
    if (preg_match($chars, $card))
    {
        return true;
    } else
    {
...
/**
 * @Author: 陈静
 * @Date: 2018/03/25 22:34:13
 * @Description: 增加Api返回格式
 */
 ...
 * @return string
 */
function outPut($code, $message, $data=[], $httpCode=200) {
 ...
 * @return bool
 */
function errOutPut($code, $message,$httpCode = 200)
{
    throw new \app\common\library\exception\ApiException($code,$message,$httpCode = 200);
}
 ...
 * @return bool
 */
function ecpOutPut($exception,$code, $message,$httpCode = 200)
{
    //写日志
    \app\wechat\common\Logs::logRecoder($exception);
    throw new \app\common\library\exception\ApiException($code,$message,$httpCode = 200);
}
...
function setHistory($q){
    if (trim($q) == '')return;
    $history_str = cookie('searchVal');
    $history_arr = !empty($history_str) ? json_decode($history_str, true) : [] ;
    $flag = null;
    ...
    if ($flag !== null) {
        unset($history_arr[$flag]);
        array_unshift($history_arr, trim($q));
        cookie('searchVal', json_encode($history_arr));
    } else {
        array_unshift($history_arr, $q);
        cookie('searchVal', json_encode($history_arr));
    }
}
