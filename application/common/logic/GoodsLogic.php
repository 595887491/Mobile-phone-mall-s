...
class GoodsLogic extends Model
{
     ...
     * @param $goods_id
     */
    public function cancel_collect_goods($user_id, $goods_id) {
        Db::name('goods_collect')->where(['goods_id' => $goods_id, 'user_id' => $user_id])->delete();
        return array('status'=>1,'msg'=>'已取消收藏','result'=>array());
    }
    /**
     * 获取商品规格
     * @param $goods_id|商品id
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        if (!empty($goods_id_arr)) $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            if ($i == 0)
                $parr[] = array('value' => "{$end}元以下", 'href' => urldecode(U("Goods/$action", $filter_param, '')),'section'=>"{$start}-{$end}" );
            elseif($i == ($c-1) && ($max_price > $end))
                $parr[] = array('value' => "{$end}元以上", 'href' => urldecode(U("Goods/$action", $filter_param, '')),'section'=>"{$start}-{$end}" );
            else
                $parr[] = array('value' => "{$start}-{$end}元", 'href' => urldecode(U("Goods/$action", $filter_param, '')),'section'=>"{$start}-{$end}" );
        }
        return $parr;
    }
    ...
    public function getFreight($goodsArr, $region_id)
    {
        ...
        //产品经理设置的非要强行加入没有经过设计的活动  新人首单  39  包邮
        //1.是否是新人
        $flag = false;
        ...
        foreach ($goodsArr as $cartKey => $cartVal) {
            //新用户累计价格
            if ( $flag ) {
                if (in_array($cartVal['goods_id'],$goodsIdArr)) {
                    $newUserFirstOrderPrice += $cartVal['goods_price'] * $cartVal['goods_num'];
                }
            }
            foreach ($goodsList as $goodsKey => $goodsVal) {
                if ($cartVal['goods_id'] == $goodsVal['goods_id']) {
                    ...
                    if ( isset($goodsArr[$cartKey]['team_free_shipping']) && $goodsArr[$cartKey]['team_free_shipping'] ) {
                        $goodsArr[$cartKey]['is_free_shipping'] = 1;
                    }
                }
            }
        }
        ...
        foreach ($template_list as $templateVal => $goodsArr) {
            ...
            $freightLogic->setGoodsPrice($this->goodsPrice);//设置订单价格
            $freightLogic->doCalculation();
            $freight += $freightLogic->getFreight();
            unset($temp);
        }
        //当为新用户，首单商品价格累计超过39   运费又大于了5（基础运费）
        if ( $flag && $newUserFirstOrderPrice > 39 && $freight >= 5) {
            $freight -= 5;
        }
        return $freight;
    }
