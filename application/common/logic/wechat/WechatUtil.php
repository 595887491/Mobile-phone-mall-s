...
class WechatUtil extends WxCommon
{
    ...
    public function getAccessToken()
    {
        ...
        $web_expires = time() + 6000; // 提前200秒过期
        Db::name('wx_user')->where('id', $wechat['id'])
            ->save(['web_access_token'=>$return['access_token'], 'web_expires'=>$web_expires]);
...
//     ]
    ...
    /**
     * @Author: 陈静
     * @Date: 2018/04/02 16:47:13
     * @Description:修改为创建临时和永久二维码，名字未动，怕其他方法调用
     */
    public function createTempQrcode($expire, $scene_id)
    {
        ...
        if ($expire) {
            $data1 = ['expire_seconds' => $expire,'action_name'    => 'QR_STR_SCENE'];
...
//        }
    ...
    public function setTemplateIndustry()
    {
        ...
        $return = $this->requestAndCheck($url, 'POST','{
          "industry_id1":"1",
          "industry_id2":"4"
       }');
        if ($return === false) {
    ...
    //设置公众号模板消息行业
    public function setTemplateIndustryInfo()
    {
        if (!$access_token = $this->getAccessToken()) {
            return false;
        }
        ...
        $return = $this->requestAndCheck($url, 'GET','{"primary_industry":{"first_class":"IT科技","second_class":"互联网|电子商务"},"secondary_industry":{"first_class":"运输与仓储","second_class":"快递"}}');
        if ($return === false) {
            return false;
        }
        return $return;
...
//            ]
    ...
    {
        $key = $GLOBALS['ENV'].'_weixin_ticket_'.$type;
