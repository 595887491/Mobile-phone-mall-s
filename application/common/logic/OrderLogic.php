...
class OrderLogic
{
        ...
        if ($_FILES['return_imgs']['tmp_name']) {
            ...
            foreach ($files as $v) {
                ...
                $res =  $ossLogic->uploadFile(
                    $info['tmp_name'] , config('aliyun_oss.img_object').'return_goods/'.date('Ymd').'/'.$new_name
                );

                if ($res == false) {
                    return ['status' => -1, 'msg' => $ossLogic->getError()];//上传错误提示错误信息
                }

                $return_imgs[]=  $ossLogic->getSiteUrl().$res;
            }
	...
	function make_virtual_code($order){
		...
		M('order')->where(array('order_id'=>$order['order_id']))->save(array('shipping_time'=>time()));
...
//        M('goods_activity')->where(array('act_id'=>$act_id))->setInc('act_count',$pre_sell_price['goods_num']);
	 ...
	 * @return string
	 */
	public function get_order_sn($type = '')
	{
	    $order_sn = null;
	    // 保证不会有重复订单号存在
	    while(true){
	        if ($type == 'partner') {
                $order_sn = date('YmdHis').rand(100,999).'p'; // 订单编号
            }else{
                $order_sn = date('YmdHis').rand(1000,9999); // 订单编号
            }
