...
class CouponLogic extends Model
{
    ...
    public function getUserAbleCouponList($user_id, $goods_ids = array(), $goods_cat_id = array())
    {
        ...
        $Coupon = new Coupon();
        $userCouponArr = [];
        $userCouponList = $CouponList->where([
            'uid'=>$user_id,
            'use_start_time'=>['<', time()],
            'use_end_time'=>['>', time()]
        ])->where('status', 0)->select();//用户优惠券

//        array_walk($userCouponList,function (&$val){
//            $val['use_start_date'] = date('Y.m.d H:i:s',$val['use_start_time']);
//        });
//        halt(collection($userCouponList)->toArray());
...
//            ->where('use_end_time', '>', time())
    ...
    /**
     * @Author: 陈静
     * @Date: 2018/04/20 14:31:6
     * @Description: 获取未登录可领取的优惠券
     */
    public function getNotLoginCoupon()
    {
        return Db::query('SELECT IF(createnum > send_num,true,FALSE ) as num FROM `tp_coupon`
WHERE  `type` = 2  AND `send_start_time` <= 1524206579  AND `send_end_time` >= 1524206579  AND `status` = 1 HAVING num >0');
    }

    /**
     * @Author: 陈静
     * @Date: 2018/04/20 15:18:9
     * @Description: 获取未使用优惠券数量
     */
    public function getNotUseCoupon($user_id)
    {
        $activityLogic = new \app\common\logic\ActivityLogic;
        $count = $activityLogic->getCouponQuery(0, $user_id, 0 )->count();
        return $count;
    }
