...
class DistributLogic //extends Model
{
     ...
     {       
         ...
         $regrade = tpCache('distribut.regrade'); // 返佣级数
		 ...
		 $data = array(             
            ...
            'money' => $first_money,'level' => 1,'create_time' => time(),             
          );  
         ...
         if($user['first_leader'] > 0 && $first_money > 0.01)
         {
            ...
            // 微信推送消息
            $first_leader = M('OauthUsers')->where(['user_id'=>$user['first_leader'] , 'oauth'=>'weixin' , 'oauth_child'=>'mp'])->find();
            if ($first_leader) {
                $wx_content = "你的一级下线,刚刚下了一笔订单:{$order['order_sn']} 如果交易成功你将获得 ￥".$first_money."奖励 !";
                $wechat->sendMsg($first_leader['openid'], 'text', $wx_content);
            }                       
         }
         // 二级 分销商赚 的钱.
     ...
     function auto_confirm(){
         ...
         $distribut_time = $distribut_date * (60 * 60 * 24); // 计算天数时间戳
    ...
    public function setStoreInfo($user_id, $storeName, $trueName, $mobile, $qq, $img = '')
    {
        // 上传图片
        $return = $this->uploadStoreImg();
        if ($return['status'] != 1) {
            return $return;
        }

        if ($return['result'] || $img) {
            $data['store_img'] = $return['result'] ?: $img;
        }
