...
class Team
{
    ...
    public function setTeamFoundById($found_id)
    {
        if($found_id){
            ...
            if ($this->teamFound->join >= $this->teamFound->need) {
                throw new TpshopException('拼团购买商品',0,['status' => 0, 'msg' => '该商品拼团活动已满员', 'result' => '']);
            }
        }
    }
    ...
    public function setUserById($user_id)
    {
        if($user_id > 0){
            ...
            //判断是否参加过拼团
            if ($this->foundId) {
                $count = Db::name('team_follow')->where('found_id',$this->foundId)
                    ->where('follow_user_id',$user_id)->where('status',1)->count();
                if ($count >= 1) {
                    throw new TpshopException('拼团购买商品',0,['status' => 0, 'msg' => '您已参加该商品拼团活动', 'result' => '']);
                }
            }
        ...
        if ($teamFound) {
            ...
            if($teamFollow){
                ...
                if($teamFollow->team_found->join >= $teamFollow->team_found->need){
                    ...
                    //发送模板消息
                    try{
                        (new WechatLogic())->sendTemplateMsgOnTeamSucess($teamFollow->team_found->found_id);
                    }catch (Exception $e){
                        Logs::sentryLogs('发送拼团成功模板消息失败:'.$e->getMessage());
                    }
                }
    ...
    {
        $payStatus = (new order)->where('order_id',$order->order_id)->getField('pay_status');
        if($this->teamFound){
            /**团员拼团s**/
            $team_follow_data = [
            ...
            //增加参团人数
            if ($payStatus && $order->order_amount) {
                Db::name('team_found')->where('found_id',$this->teamFound['found_id'])->setInc('join');
                //查询多少人数
                $teamFoundInfo = Db::name('team_found')->where('found_id',$this->teamFound['found_id'])->find();
                //拼团成功
                if ( $teamFoundInfo['join'] == $teamFoundInfo['need'] ) {
                    //修改状态
                    Db::name('team_found')->where('found_id',$teamFoundInfo['found_id'])->where('status',1)->update([
                        'status' => 2
                    ]);
                    Db::name('team_follow')->where('found_id',$teamFoundInfo['found_id'])->where('status',1)->update([
                        'status' => 2
                    ]);
                    //发送模板消息
                    try{
                        (new WechatLogic())->sendTemplateMsgOnTeamSucess($teamFoundInfo['found_id']);
                    }catch (Exception $e){
                        Logs::sentryLogs('发送拼团成功模板消息失败:'.$e->getMessage(),$teamFoundInfo);
                    }
                }
            }
            /***团员拼团e***/
