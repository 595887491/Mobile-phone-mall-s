...
class CartLogic extends Model
{
    ...
    public function setGoodsModel($goods_id)
    {
        if($goods_id > 0){
            ...
            //兼容之前的活动数据
            if ($this->goods['prom_type'] == 1 || $this->goods['prom_type'] == 3) {
                $this->goods['prom_type'] = 0;
                $this->goods['prom_id']     = 0;
            }
            $ActivityLogic = new ActivityLogic();
            $activity = $ActivityLogic->goodsRelatedActivity($goods_id);
            if ($activity) {
                $this->goods['prom_type'] = $activity['prom_type'];
                $this->goods['prom_id']     = $activity['prom_id'];
            }
        }
    }
    /**
    ...
    public function setSpecGoodsPriceModel($item_id)
    {
        if($item_id > 0){
            ...
            //兼容之前的活动信息
            if ($this->specGoodsPrice['prom_type'] == 1 || $this->specGoodsPrice['prom_type'] == 3) {
                $this->specGoodsPrice['prom_type']   = 0;
                $this->specGoodsPrice['prom_id']     = 0;
            }
            $ActivityLogic = new ActivityLogic();
            $activity = $ActivityLogic->goodsRelatedActivity($this->goods['goods_id'],$item_id);
            if ($activity) {
                $this->goods['prom_type']   = $activity['prom_type'];
                $this->goods['prom_id']     = $activity['prom_id'];
                $this->specGoodsPrice['prom_type']   = $activity['prom_type'];
                $this->specGoodsPrice['prom_id']     = $activity['prom_id'];
            }
        }
    }
    ...
    private function addFlashSaleCart(){
        ...
        //获取用户购物车的抢购商品
        if (!$this->user_id) {
            $userCartGoods = Cart::get(['user_id'=>$this->user_id,'session_id'=>$this->session_id,'goods_id'=>$this->goods['goods_id'],'spec_key'=>($this->specGoodsPrice['key'] ?: ''),'prom_type'=>$this->goods['prom_type'],'prom_id'=>$this->goods['prom_id']]);
        } else {
            $userCartGoods = Cart::get(['user_id'=>$this->user_id,'goods_id'=>$this->goods['goods_id'],'spec_key'=>($this->specGoodsPrice['key'] ?: ''),'prom_type'=>$this->goods['prom_type'],'prom_id'=>$this->goods['prom_id']]);
        }
        $userCartGoodsNum = empty($userCartGoods['goods_num']) ? 0 : $userCartGoods['goods_num'];///获取用户购物车的抢购商品数量
        ...
        if($userCartGoods){
            ...
            if($this->specGoodsPrice){
                ...
                $cartAddFlashSaleData['market_price'] = $this->specGoodsPrice['price'];   // 规格价
    ...
    private function addPromGoodsCart(){
        ...
        $promGoodsItem = Db::name('prom_goods_list')->where(['promote_id'=>$promGoods['id'],'goods_id'=>$this->goods['goods_id']])->where(function ($query){
            if ($this->specGoodsPrice) $query->where('item_id',$this->specGoodsPrice['item_id']);
        })->find();
        if ($promGoodsItem) {
            if ($promGoodsItem['goods_num'] < $promGoodsItem['buy_num'] + $userWantGoodsNum) {//参与活动商品数量=已售出活动商品数量+用户想购买的数量
                return array('status' => -4, 'msg' => '购物车已有'.$userCartGoodsNum.'件，不能购买更多活动商品', 'result' => '');
            }
        }
        $userWantGoodsNum = $this->goodsBuyNum + $userCartGoodsNum;//本次要购买的数量加上购物车的本身存在的数量
        if($userWantGoodsNum > 200){
        ...
        if ($userCartGoods) {
           /* $userWantGoodsNum = $this->goodsBuyNum + $userCartGoods['goods_num'];//本次要购买的数量加上购物车的本身存在的数量
            ...
            if($this->specGoodsPrice){
                $cartAddData['spec_key'] = $this->specGoodsPrice['key'];
                $cartAddData['market_price'] = $this->specGoodsPrice['price'];
                $cartAddData['spec_key_name'] = $this->specGoodsPrice['key_name']; // 规格 key_name
                $cartAddData['sku'] = $this->specGoodsPrice['sku']; //商品条形码
    ...
    public function checkCartList($cartList){
        ...
        foreach($cartList as $cartKey=>$cart){
            ...
            if ($goodsPromFactory->checkPromType($cart['prom_type'])) {
                //检测现在的活动，和购物车的活动是否相同，不相同就删除
                if ($cart['spec_key']) {
                    $item_id = Db::name('spec_goods_price')->where(['key'=>$cart['spec_key'],'goods_id'=>$cart['goods_id']])->getField('item_id');
                    $activity = $ActivityLogic->goodsRelatedActivity($cart['goods_id'],$item_id);
                } else {
                    $activity = $ActivityLogic->goodsRelatedActivity($cart['goods_id']);
                }
//                halt($activity);
                if ($activity) {
                    if ($activity['prom_id'] != $cart['prom_id'] || $activity['prom_type'] != $cart['prom_type']) {
                        $cart->delete();
                        unset($cartList[$cartKey]);
                        continue;
                    }
    ...
    public function getCouponCartList($cartList, $userCouponList)
    {
        ...
        foreach ($cartList['cartList'] as $v) {
            if ($v['prom_type'] == 1 || $v['prom_type'] == 2 ) {
                $totalSeckillPrice += $v['member_goods_price'] * $v['goods_num'];
            }
        }
        ...
        foreach ($userCouponArray as $couponKey => $couponItem) {
            if ($userCouponArray[$couponKey]['coupon']['use_type'] == 0) { //全店使用优惠券
                //订单商品总价是否符合优惠券购买价格
                if ($cartList['total_fee'] - $totalSeckillPrice >= $userCouponArray[$couponKey]['coupon']['condition']) {
