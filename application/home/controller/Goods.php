...
class Goods extends Base {
    ...
    */ 
    ...
    public function dispatching()
    {
        ...
        $shop_price = input('post.shop_price',0);
        $Goods = new \app\common\model\Goods();
        ...
        $freightLogic->setGoodsPrice($shop_price);
...
//        $replyList = M('Comment')->where(['is_show'=>1,'goods_id'=>$goods_id,'parent_id'=>['>',0]])->order("add_time desc")->select();
    ...
    public function collect_goods(){
        ...
        $goodsLogic = new GoodsLogic();
        $result = [];
        // 如果只有一个商品时，点击收藏，再次点击取消收藏
        if (count($goods_ids) == 1 && is_numeric($goods_ids[0])) {
            $is_collect = $goodsLogic->isCollectGoods($user_id, $goods_ids[0]);
            if ($is_collect == 1) {
                $goodsLogic->cancel_collect_goods($user_id, $goods_ids[0]);
                $this->ajaxReturn(['status'=>1,'msg'=>'已取消收藏','action'=>0,'result'=>$result]);
            } else {
                $goodsLogic->collect_goods($user_id, $goods_ids[0]);
                $this->ajaxReturn(['status'=>1,'msg'=>'已收藏该商品','action'=>1,'result'=>$result]);
            }
        }
        foreach($goods_ids as $key=>$val){
            $result[] = $goodsLogic->collect_goods($user_id, $val);
        }

        $this->ajaxReturn(['status'=>1,'msg'=>'已收藏该商品','result'=>$result]);
    }
