...
class Api extends Base {
    ...
    public function queryExpress()
    {
    	...
    	if($express_switch == 1){
    		...
    		if(empty($shipping_code) || empty($invoice_no)){
    			return json(['status'=>0,'message'=>'订单号错误','result'=>'']);
    		}
    ...
    public function agentSubArea(){
        $area_id = input('area_id',0, 'intval');
        if (empty($area_id)) exit(json_encode([]));

        $area_agent = Db::table('tp_region r')
            ->field('r.id, r.name region_name, ug.*, u.nickname')
            ->join(['cf_user_agent'=> 'ug'],'r.id=ug.region_id','left')
            ->join(['tp_users'=> 'u'],'u.user_id=ug.user_id','left')
            ->where('r.parent_id',$area_id)
            ->where('is_enabled_agent',1)
            ->select();
        exit(json_encode($area_agent));
    }
}