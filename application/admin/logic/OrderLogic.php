...
class OrderLogic
{
    ...
    public function getOrderList($condition,$order='',$start=0,$page_size=20){
        $res = M('order')
            ->field('o.*, u.nickname, u.mobile as user_mobile,b.act_type')
            ->alias('o')
            ->join(['tp_users'=>'u'],'o.user_id=u.user_id','left')
            ->join(['cf_bargain_found'=>'a'],'a.order_id=o.order_id','left')
            ->join(['cf_bargain_activity'=>'b'],'a.bargain_id=b.id','left')
            ->where($condition)
    ...
    public function orderProcessHandle($order_id,$act,$ext=array()){
    	...
    	switch ($act){
    		case 'pay': //付款
               	...
               	//添加分成
//                $result['out_trade_no'] = $orderInfo['order_sn'];
//                $result['total_fee'] = ($orderInfo['order_amount'] + $orderInfo['user_money']) * 100;
//                $result['openid'] = 'user_id:'.$orderInfo['user_id'];
//                $result['msg'] = '后台操作直接付款';
//
//                //先检查是否有此订单信息
//                $orderPayLogInfo = (new PayLogModel())->where('pay_code', '=', 'admin_action')
//                    ->where('trade_num', '=', $result['out_trade_no'])->find();
//
//                if (empty($orderPayLogInfo)){
//                    //插入支付日志
//                    (new PayLogModel())->insertPayLog($result,'admin_action');
//
//                    //申请合伙人逻辑
...
//                }
                ...
                $userInfo = (new \app\common\model\Order())->alias('a')
                    ...
                    ->where('a.order_sn',$orderInfo['order_sn'])->find();
                if ($userInfo && !config('APP_DEBUG')) {
                    (new SmsLogic())->sendSms(5,$userInfo->mobile,['name' => $userInfo->nickname,'order_sn' => $orderInfo['order_sn']]);
                }

                update_pay_status($orderInfo['order_sn'],$ext); // 调用确认收货按钮
    			return true;    			
    ...
    public function deliveryHandle($data){
        ...
        /**
         * @Author: 陈静
         * @Date: 2018/05/12 8:33:56
         * @Description: 取消后台发货短信提醒
         */
		//商家发货, 发送短信给客户
...
//		}
        ...
        $res =$wechat->sendTemplateMsgOnDeliver($data);
        if ($res['status'] == -1) {
    ...
    public function submitOrderExpress($data,$orderGoods){
    	...
    	$region = M('region')->where(array('id'=>array('in',$region_ids)))->getField('id,name');
