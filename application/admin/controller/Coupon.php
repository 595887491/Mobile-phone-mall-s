...
class Coupon extends Base {
    ...
    public function coupon_info(){
        ...
        if ($cid) {
            ...
            if (empty($coupon)) {
                ...
                $coupon['valid_time'] = $coupon['valid_time']/86400;
            }
        ...
        $needInviteMembers = tpCache('distribute.invite_members_threshold');

        $this->assign('invite_members',$needInviteMembers);
        return $this->fetch();
    }
    ...
    public function addEditCoupon()
    {
        ...
        $data['valid_time'] = $data['valid_time'] * 86400;
        if (empty($data['id'])) {
            $data['add_time'] = time();
    			...
    			if($user){
    				...
    				foreach ($user as $k=>$val){
                        ...
                        $insert[] = [
                            ...
                            'send_time' => $time,
                            'use_start_time' => time(),
                            'use_end_time' => time() + $coupon['valid_time']
                        ];
    				}
    			}
    		}else{
