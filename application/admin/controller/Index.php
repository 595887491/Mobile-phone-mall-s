...
//    	$today = strtotime("-1 day");
    	...
    	$count['handle_order'] = M('order')->where("order_status=0 and (pay_status=1 or pay_code='cod') and prom_type <>6")->count();//待处理普通订单订单
    ...
    public function changeTableVal(){
            ...
            $value  = I('value'); // 修改字段值
            $isTpTable  = I('is_tp_table'); // 修改字段值
            if ($isTpTable) {
                $res = Db::table($table)->where("$id_name = $id_value")->save(array($field=>$value)); // 根据条件保存修改的数据
                halt($res);
            }else{
                M($table)->where("$id_name = $id_value")->save(array($field=>$value)); // 根据条件保存修改的数据
            }
            if ($table == 'goods') (new EsModel())->initialization();
    }
