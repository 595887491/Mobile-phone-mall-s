...
class Goods extends Base {
    ...
    public function export_goods(){
        $where = ' 1 = 1 '; // 搜索条件
        I('intro')    && $where = "$where and ".I('intro')." = 1" ;//新品、推荐
        I('brand_id') && $where = "$where and brand_id = ".I('brand_id') ;//品牌ID
        (I('is_on_sale') !== '') && $where = "$where and is_on_sale = ".I('is_on_sale') ;
        $cat_id = I('cat_id');
        // 关键词搜索
        $key_word = I('key_word') ? trim(I('key_word')) : '';
        ...
        $order_str = "`{$_POST['orderby1']}` {$_POST['orderby2']}";
        $goodsList = Db::name('goods g')->field('g.*,gc.name cat_name, gc.mobile_name,gc.parent_id_path,b.name brand_name')
            ->join('goods_category gc','gc.id=g.cat_id','left')
            ->join('brand b','b.id=g.brand_id','left')
            ->where($where)->order($order_str)
            ->select();
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;width:120px;" >条码/货号</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:50px;" >本店价</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:50px;" >市场价</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:150px;" >活动价/活动名</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:120px;" >品牌</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:40px;" >库存</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:80px;" >大类</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:80px;" >中类</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:80px;" >小类</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:40px;" >是否推荐</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:40px;" >是否新品</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:50px;" >商品销量</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:40px;" >是否上架</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:250px;" >属性价格</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:250px;" >商品介绍</td>';
        $strTable .= '</tr>';
        if(is_array($goodsList)){
            $goodsPromFactory = new GoodsPromFactory();
            foreach($goodsList as $k=>$val){
                $val['is_recommend'] = $val['is_recommend'] ? '是': '否';
                $val['is_new'] = $val['is_new'] ? '是': '否';
                $val['is_on_sale'] = $val['is_on_sale'] ? '是': '否';
                $parent_path = explode('_',$val['parent_id_path']);//分类族谱
                $parent_cat = Db::name('goods_category')->where(['id'=>['in',$parent_path]])->getField('name', true);
                $parent_cat = array_reverse($parent_cat);
                // 规格 对应 价格 库存表
                $spec_goods_price  = M('spec_goods_price')->where("goods_id", $val['goods_id'])->getField("key,price,sku,store_count,item_id");
                if ($spec_goods_price) {
                    $rowspan = count($spec_goods_price);
                    $spec_item_id = [];
                    array_walk($spec_goods_price,function($val) use (&$spec_item_id) {
                        $items = explode('_',$val['key']);
                        $spec_item_id = array_merge($spec_item_id, $items);
                    });
                    $spec_item_id = array_unique($spec_item_id);
                    $spec_item = Db::name('spec_item i')
                        ->join('spec s','s.id = i.spec_id','left')
                        ->field('i.id, i.item ,s.name')
                        ->where(['i.id'=>['in',$spec_item_id]])
                        ->select();
                    foreach ($spec_item as $k => $item){
                        $spec_item_new[$item['id']] = $item;
                    }
                    unset($spec_item);
                } else {
                    $rowspan = 1;
                ...
                $val['activity_is_on'] = $goods['activity_is_on'];
                $val['goods_remark'] = $goods['goods_remark'];
                $val['activity_price'] = $goods['activity_is_on'] ? $goods['shop_price'].'/'.$goods['activity_title']: '--';
                unset($goods);

                $strTable .= '<tr>';
                $strTable .= '<td style="text-align:center;font-size:12px;"rowspan="'.$rowspan.'">'.$val['goods_id'].'</td>';
                $strTable .= '<td style="text-align:left;font-size:12px;"rowspan="'.$rowspan.'">'.$val['goods_name'].' </td>';
                $strTable .= '<td style="text-align:left;font-size:12px;"rowspan="'.$rowspan.'">'.$val['goods_sn'].' </td>';
                $strTable .= '<td style="text-align:center;font-size:12px;"rowspan="'.$rowspan.'">'.$val['shop_price'].'</td>';
                $strTable .= '<td style="text-align:center;font-size:12px;"rowspan="'.$rowspan.'">'.$val['market_price'].'</td>';
                $strTable .= '<td style="text-align:center;font-size:12px;"rowspan="'.$rowspan.'">'.$val['activity_price'].'</td>';
                $strTable .= '<td style="text-align:left;font-size:12px;"rowspan="'.$rowspan.'">'.$val['brand_name'].' </td>';
                $strTable .= '<td style="text-align:left;font-size:12px;"rowspan="'.$rowspan.'">'.$val['store_count'].'</td>';
                $strTable .= '<td style="text-align:left;font-size:12px;"rowspan="'.$rowspan.'">'.$parent_cat[2].'</td>';
                $strTable .= '<td style="text-align:left;font-size:12px;"rowspan="'.$rowspan.'">'.$parent_cat[1].'</td>';
                $strTable .= '<td style="text-align:left;font-size:12px;"rowspan="'.$rowspan.'">'.$parent_cat[0].'</td>';
                $strTable .= '<td style="text-align:center;font-size:12px;"rowspan="'.$rowspan.'">'.$val['is_recommend'].'</td>';
                $strTable .= '<td style="text-align:center;font-size:12px;"rowspan="'.$rowspan.'">'.$val['is_new'].'</td>';
                $strTable .= '<td style="text-align:center;font-size:12px;"rowspan="'.$rowspan.'">'.$val['sales_sum'].'</td>';
                $strTable .= '<td style="text-align:center;font-size:12px;"rowspan="'.$rowspan.'">'.$val['is_on_sale'].'</td>';
                if ($spec_goods_price) {
                    $first_row = array_splice($spec_goods_price, 0 ,1);
                    foreach ($first_row as $price_k => $v){
                        $spec_item_id = explode('_',$v['key']);
                        $spec_desc = '';
                        foreach ($spec_item_id as $item_id){
                            $spec_desc .= $spec_item_new[$item_id]['name']."：".$spec_item_new[$item_id]['item']." ";
                        }
                        $strTable .= '<td style="font-size:12px;">'.$spec_desc.' 价格：'.$v['price'].'，sku:'.$v['sku'].'</td>';
                    }
                }else{
                    $strTable .= '<td style="text-align:center;font-size:12px;"rowspan="'.$rowspan.'"></td>';
                }
                $strTable .= '<td style="text-align:center;font-size:12px;"rowspan="'.$rowspan.'">'.$val['goods_remark'].'</td>';
                $strTable .= '</tr>';
                if ($spec_goods_price) {
                    foreach ($spec_goods_price as $price_k => $v){
                        $spec_item_id = explode('_', $v['key']);
                        $spec_desc = '';
                        foreach ($spec_item_id as $item_id) {
                            $spec_desc .= $spec_item_new[$item_id]['name'] . "：" . $spec_item_new[$item_id]['item'] . " ";
                        }
                        $strTable .= '<tr><td style="font-size:12px;">' . $spec_desc . ' 价格：' . $v['price'] . '，sku:' . $v['sku'] . '</td></tr>';
                    }
                }
            }
        }
    ...
    {
        $EsSearch = new EsModel();//elasticsearch模型
        $GoodsLogic = new GoodsLogic();
...
//            if ($type == 2){
////                $EsSearch->updateIndex($goods_id,I('goods_name'),I('keywords')); //更改产品后更新索引库
//            }else{
////                $EsSearch->addIndex($goods_id,I('goods_name'),I('keywords')); //新增产品后更新索引库
//            }
            $EsSearch->initialization();
...
//        $brandList = $GoodsLogic->getSortBrands($goodsInfo['cat_id']);   //获取三级分类下的全部品牌
        $brandList = $GoodsLogic->getSortBrands(0);   //获取三级分类下的全部品牌
        ...
        $goodsImages = M("GoodsImages")->where('goods_id =' . I('GET.id', 0))->order('img_id DESC')->select();
    ...
    public function brandList(){
        $where = [];
        $keyword = I('keyword');
        $typeTab = I('typeTab');//0为全部;1 已推荐;2 未推荐
        if($typeTab == 1) $where['is_hot'] = 1;
        if($typeTab == 2) $where['is_hot'] = 0;
        if ($keyword) $where['name'] = ['like',"%$keyword%"];
        $count = Db::name("Brand")->where($where)->count();
        if ($count ==0){//搜索长馆名无果时收索国家
            unset($where['name']);
            $country = (new GoodsGlobal())->searchCountry($keyword);
            $where['country_id'] = array('in',$country);
            $count = Db::name("Brand")->where($where)->count();
        }
        ...
        for($i=0;$i<count($brandList);$i++){
            $brandList[$i]['country'] = Db::table('cf_country')->field('name')->where('country_id',$brandList[$i]['country_id'])->find()['name'];
            $brandList[$i]['goodsNum'] = M('goods')->where('brand_id',$brandList[$i]['id'])->count();
        }
        $show  = $Page->show(); 
        ...
        $this->assign('brandList',$brandList);
        if (IS_AJAX){
            return $this->fetch('ajax_brandList');
        }
        return $this->fetch('brandList');
    }

    //创建索引
    public function cfCreatingIndex(EsModel $EsSearch){
        $EsSearch->creating();
    }
...
//                }
            ...
            $country_list = Db::table('cf_country')->where('status',1)->select();
           $cat_list = M('goods_category')->where("parent_id = 0")->select(); // 已经改成联动菜单
           $this->assign('cat_list',$cat_list);           
           $this->assign('country_list',$country_list);//国家下拉菜单
     ...
     * 修改品牌链接
     * */
    public function editBrandUrl()
    {
        $brandId = I('id');
        $url = I('url');
        $res = Db::table('tp_brand')->where('id',$brandId)->update(['url'=>$url]);
        if ($res){
            return json(['data'=>1,'msg'=>'修改品牌链接成功']);
        }else{
            return json(['data'=>-1,'msg'=>'修改品牌链接失败']);
        }
    }
    ...
    /*****************    索引库操作-赵磊    ****************/
    //更新索引
    public function initialization(EsModel $EsSearch)
    {
        $res = $EsSearch->deleteDoc();
        if ($res) $result = $EsSearch->creating();
//        if ($result) return '更新成功';
    }

    //删除索引
    public function delIndex(EsModel $EsSearch){
        $EsSearch->deleteDoc();
    }
    /*****************          end          ****************/
}