/**
 2  * zepto插件：向左滑动删除动效
 3  * 使用方法：v('.itemWipe').touchWipe({itemDelete: '.item-delete'});
 4  * 参数：itemDelete  删除按钮的样式名
 5  */
(function (v) {
    v.fn.touchWipe = function (option) {
        var defaults = {
            itemDelete: '.item-delete', //删除元素
        };
        var opts = v.extend({}, defaults, option); //配置选项
        var delWidth = v(opts.itemDelete).width();
        ...
        v(this).on('touchstart', function (event) {
            // console.log('start..');
            var obj = this;
            ...
            // console.log(objX);
            if (objX == 0) {
                v(this).on('touchmove', function (event) {
        ...
        //结束时判断，并自动滑动到底或返回
        v(this).on('touchend', function (event) {
            var obj = this;
            objX = (obj.style.WebkitTransform.replace(/translateX\(/g, "").replace(/px\)/g, "")) * 1;
            if (objX > -delWidth / 2) {
