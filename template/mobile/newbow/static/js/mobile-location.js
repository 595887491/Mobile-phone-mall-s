...
function ajaxDispatching() {
  ...
  var shop_price = $("input[name='shop_price']").val();//传入价格方便计算运费
  var region_id = district_id;
  if(typeof(goods_id) != 'undefined' && region_id!= ''){
    $.ajax({
      type: "POST",
      dataType: 'json',
      data: {goods_id: goods_id, region_id: region_id,shop_price:shop_price},
      url: "/index.php?m=Home&c=Goods&a=dispatching",
      success: function (data) {
        if (data.status == 1) {
            //有货
            // if(data.result.freight == 0){
            //     $('#shipping_freight').text('包邮');
            // }else{
            //     $('#shipping_freight').text('￥'+data.result.freight);
            // }
